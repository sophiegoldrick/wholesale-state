...
if GEN_TYPE in ('prints','all'):
    ...
    # ── SPECIAL customers: one fronts + backs file per customer ──
    ...
    if sr_special:
        ...
        for r in sr_special:
            ...
            if cid not in special_customers:
                special_customers[cid] = {'name': r.get('Customer','Unknown'), 'rows': []}
            special_customers[cid]['rows'].append(r)
        for cid, cd in special_customers.items():
            # Safe filename: strip non-alphanumeric, max 30 chars
            import re as _re
            safe = _re.sub(r'[^a-zA-Z0-9\s]','',cd['name']).strip().replace(' ','_')[:30]
            crows = cd['rows']
        ...
        print(f'✅ SPECIAL prints — {len(special_customers)} customer(s)', file=sys.stderr)
